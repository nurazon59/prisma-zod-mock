...
export function analyzeFieldName(fieldName: string): FieldAnalysis {
  ...
  if (lowerFieldName.includes('bio') || lowerFieldName.includes('description'))
  ...
  else if (patterns.includes('name')) inferredType = 'name';
  else if (patterns.includes('description')) inferredType = 'description';
