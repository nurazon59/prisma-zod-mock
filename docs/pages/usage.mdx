...
describe('User Service', () => {
  ...
  it('should handle batch operations', async () => {
    ...
    const results = await Promise.all(mockUsers.map((user) => createUser(user)));
...
export const handlers = [
  rest.get('/api/users/:id', (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(createUserMock({ id: req.params.id })));
  }),

  rest.get('/api/posts', (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(createPostMockBatch(20)));
  }),
];
```
